...
takes in an array of strings and uses Regex
 to create a new array of only those strings 
...
const screenForNames = (arr) => {
  let newArr = arr.filter((item)=>{
    if(item.charAt(0) !== ' ' && !item.includes('Mr.  P'))
    {
      if(item.includes('Mr. ') || item.includes('Mrs. ') || item.includes('Ms. ') || item.includes('Dr. '))
      {
        return item
      }
    }
...
an array of strings with the first character 
in upper case and the rest as is.
...
given the Star Wars data, below, returns the 
names of the characters whose mass is greater than Luke's.

The names should be combined 
into a single string with each 
character name separated by a dash.
...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
Write a function named sortBy that takes in 
an array of objects, each of which has a particular 
property, and sorts those objects by that property, 
...
given URL is secure, beginning with https://
...
Instead of trying to write crazy for 
loops to automate checking the rows, columns 
and diagonals consider writing one helper function 
that accepts three coordinate pairs and checks the values 
of the array at those locations. 
